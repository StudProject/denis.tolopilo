...
#define CANNON_A 400
#define CANNON_CD 2000 //заряд в миллисекундах
#define WINDOW_WIDTH 800
#define WINDOW_HEIGHT 600
#define LAND_HEIGHT 85
#define ALPHA_MIN 30
#define ALPHA_MAX 150
#define FILE_NAME "save.game"// имя файла в который будет сохранятся игра, создастся в директории с игрой

TCannon cannon;//орудие
TSkydiver skydrivers[SKYDIVER_COUNT]; // парашютисты
int level = 1; // уровень игры характеризующий скорость падению парашютистов
bool FlagStop=false; // флаг остановки игры, выставляется в true, когда игра была остановлена, например при открытии модального окна.
int c=1;
...
{   
	srand(GetTickCount());
	// создание парашютистов в определенной области
	RECT r = {0,0,800,20};
	for (int i=0;i<SKYDIVER_COUNT;i++)
	{
	...
	cannon.X = 400; //абсцисса пушки
	cannon.a = 0;
	cannon.v = 0;
	cannon.health = 3; //здоровье пушки
	cannon.alpha=90; //начальный угол орудия 90
	cannon.triggerOn=false; //изначально орудие выключено
	cannon.cannonCD=2000; // и имеет полный запас энергии
...
{
	RECT r = {0,0,800,20};
	// пповышение уровня после каждых 10 набранных очков
	...
	for (int i=0;i<SKYDIVER_COUNT;i++)
	{
		...
		if(skydrivers[i].status==false) //если летит
			skydrivers[i].Y += ((double)dT/1000)*skydrivers[i].dY;// пересчитывание ординаты
		else{ //идёт
			skydrivers[i].X += ((double)dT/1000)*skydrivers[i].dX;// пересчитывание ординаты
		}
	...
	if (cannon.triggerOn) //если пушка включена
	{
		...
		double tgA = 0;
		if(cannon.alpha!=90)// для 90 тангенс не определён
			tgA=sin(radA)/cos(radA); // тангенс угла
		if(cannon.cannonCD<1){// если энергии не осталось то выключить пушку
			cannon.triggerOn=false;
		}
		if(cannon.alpha==90){//если угол пушки 90 то подсчет не требуется
			cannon.traceX = cannon.X;
			cannon.traceEnd = 0;
		}
		else{
			// рассчет в какую сторону поля упрётся луч пушки, если не встретит препятствий, и вычисление его длинны
			double len=0;
			...
			//вычисление координаты конца луча
		...
		for (int i=0;i<SKYDIVER_COUNT;i++)
		{
			if(cannon.alpha==90){//если угол поворота 90 то тогда просто ищим шары с той же абсциссой
				...
				if(cannon.alpha<90){//если угол меньше 90
					...
					int y=0;
					while(x<(skydrivers[i].X+skydrivers[i].r)){//движение от левой части парашютиста и до его левой части по линни луча и проверка каждой точки  на вхождение в парашютиста
						x++;
		...
		if(cannon.cannonCD!=CANNON_CD){
			if(cannon.cannonCD<=CANNON_CD)//продолжение зарядки
				cannon.cannonCD += (dT/10);
			else{//удаление лишнего заряда орудия
...
//
//  Назначение:  Обработка сообщений из главного окна.
//
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		switch (wParam)
		{
		case 'A':  //поворот  ствола влево
			cannon.a = 0;
			break;
		case 'D':
			cannon.a = 0; //поворот ствола вправо
			break;
		case ' ':
			cannon.triggerOn = false; //выстрел
			break;
	...
	case WM_PAINT:
		...
		//рисование парашютистов
		POINT skydraw[3];//массив с координатами для отрисовки треугольника парашютиста
		for (int i=0;i<SKYDIVER_COUNT;i++)
		{
			if(skydrivers[i].status==false){//если парашютист летит, то рисование треугольника
				...
				Chord(backbuffDC,skydrivers[i].X-skydrivers[i].r, skydrivers[i].Y-skydrivers[i].r, skydrivers[i].X +skydrivers[i].r, 
					skydrivers[i].Y+skydrivers[i].r,skydrivers[i].X+skydrivers[i].r, skydrivers[i].Y ,skydrivers[i].X-skydrivers[i].r, skydrivers[i].Y);//закрашивание нижней части круга
				...
				hOldPen = (HPEN)SelectObject(backbuffDC,hPen); // выбираем обводку
				hBrush = CreateSolidBrush(RGB(0,0,0));// создание кисти 
				hOldBrush = (HBRUSH)SelectObject(backbuffDC,hBrush);
				...
				hOldBrush = (HBRUSH)SelectObject(backbuffDC,hBrush);
				skydraw[0].x = skydrivers[i].X-skydrivers[i].r;//заполнение координат треугольника
		...
		if(cannon.destroy ==true){
			if(MessageBox(hWnd,_T("Желаете попробовать снова?"), _T("Вы проиграли."), IDOK)==IDOK){
				FlagStop=true;
